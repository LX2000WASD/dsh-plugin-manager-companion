 ...
 * 那会让样式静默丢失。
 *
 * 注意这一行 querySelector 的转义：字符串里必须出现真正的单引号（拼接后的选择器是
 * style[data-plugin-css="<tagId>"]），**不能**出现反斜杠转义的单引号——那会让产物语法错误。
 *
 ...
 *
 * 类名哈希用官方同款 pattern `[hash]_[local]`，使产物形态与官方插件一致，便于排查对照。
 *
...
function cssModulesInline() {
  return {
    ...
    async load(this: { addWatchFile(file: string): void }, virtualId: string) {
      ...
      for (const [local, exp] of Object.entries(result.exports ?? {}).sort(
        ([left], [right]) => (left < right ? -1 : left > right ? 1 : 0),
      )) {
        classMap[local] = String(exp.name)
      }
      return styleInjectionModule(PACKAGE_ID, fileId, result.code.toString(), classMap)
    },
  }
}
...
export default defineConfig({
  ...
  format: 'cjs',
  platform: 'browser',
  // 类型由 tsc 产出（tsconfig.client.json）；dts 会把 banner/footer 包进 .d.cts 破坏解析。
  dts: false,
  clean: false,
  sourcemap: false,
  external: PLATFORM,
