...
| B6 | `initProfile(dir, bundles)`（**两参**）+ `PROFILE_TEMPLATES` + `DEFAULT_PROFILE_BUNDLES` | `src/envManager.ts:38`（import）、建环境路径 | **明确报错**（签名变化）；模板/默认 bundle 集变化 → **静默**（新建环境可能直接起不来） | `node tools/e2e-lifecycle.mjs`（真建环境并启动；14/14 已跑通）；`node --test tests/envManager.test.mjs` | 官方 `packages/boot/app-boot/src/profile.ts:197`（initProfile）、`:135`（PROFILE_TEMPLATES）、`:159`（DEFAULT_PROFILE_BUNDLES） | 已实测（task-62：`node tools/e2e-lifecycle.mjs` 自建 profile 真机 14/14，含无 PATH 复跑） |
...
| C4 | **依赖兜底目录的维护语义**：`$DSH_HOME/profiles/node_modules` 由官方 `healProfilesModuleFallback` 维护，**只增不删**（判据 `moduleFallbackCurrent` = `entries.every(...)`，`healProfilesModuleFallbackLocked` 只遍历 `entries`，无 readdir/unlink）。**0.1.6-alpha.2 起默认 `resolutionMode = "runtime"`**，runtime 走 `createProfileResolutionGeneration` → `materialize: false`，**不写这个目录**，链接只从内存 generation 解析 | `src/moduleFallback.ts`（新，诊断与清理）、`src/diagnostics.ts`（L1） | **静默**：官方若改回写盘或换解析方式，我们的"陈旧链接"口径会失准；官方若开始清理，我们的清理就成了重复劳动 | 全新临时 `DSH_HOME` 起实例，看 `profiles/node_modules` 链接数是否为 0（runtime 特征）；或读 `apps/cli/src/profile-boot.ts` 的 `resolutionMode` 默认值 | 官方 `packages/boot/app-boot/src/profile.ts:543`（`entries.every`）、`:661`（只遍历 entries）、`:633`（`materialize: false`）；`apps/cli/src/profile-boot.ts:263`（默认 runtime） | 已实测（2026-09-20：全新临时 HOME 起实例 → 链接数 0；真机 `~/.dsh` 有 **516** 条链接**全部断链**，指向已删除的旧代际。**口径 = 顶层 171 + `@scope/` 下一层 345**；只数顶层（`maxdepth 1`）会得到 171，两个数都对但含义不同） |

| C5 | **官方私有层清理不完整**：`<profile>/.dsh-module-fallback` 由官方 `healProfileModuleFallback` 维护，它**有**反向差集清理（`for (const packageName of ownedPackageNames(ownedModulesDir)) if (!links.has(packageName)) removeProfileSymlink(...)`）。但真机实测发现**有机制却没清干净**：`~/.dsh/profiles/web/.dsh-module-fallback/node_modules` 里留下 **2 条断链**（`react`、`loose-envify`，都指向已不存在的 `<profile>/node_modules/<name>`）| `src/moduleFallback.ts`（`scanProfileModuleFallback`）、`src/diagnostics.ts`（`profile-module-fallback-dangling-link`，report-only，**无 fix**：那目录归官方管，我们只报不代清）| **官方若修好**：我们那条 issue 变成永不触发的死代码（不是坏事，但要知道能删）；**官方若进一步不管**：这类残留会一直累积，用户只能手工清 | 跑一次体检看 `profile-module-fallback-dangling-link` 是否出现；或 `find ~/.dsh/profiles/*/.dsh-module-fallback/node_modules -mindepth 1 -maxdepth 2 -type l ! -exec test -e {} \; -print` | 官方 `packages/boot/app-boot/src/profile.ts`（`healProfileModuleFallback` 的 `removeProfileSymlink` 循环）、`:462`（`removeProfileSymlink` 本体）| 已实测（2026-09-20 真机：`web` 2 条断链；`headless` 0 条、`cu-test` 0 条 —— 只有 `web` 有。私有层里另 3 条 `@deepseek-ai/dsh-experimental-*` 目标都在，正常）|
...
| D7 | 官方语义令牌的**浅色**取值：`--dsw-alias-label-tertiary` 白底 3.71:1、`--dsw-alias-link` 白底 4.23:1（暗色同一批为 8.54 / 6.86） | 我们**主动保持官方原样**的两处：未选中 tab `.tab`、市场「打开仓库」`.link`（`src/client/ConsolePage.module.css:40`、`src/client/MarketplacePage.module.css:115`） | **静默**：官方若把这两个值抬到 ≥4.5，本行登记与我们的实测口径都要重跑；我方承重文本用 `--dsw-alias-label-secondary`（浅色 5.8:1），不受影响 | `node --test tests/console-page.test.mjs`（视觉令牌护栏钉层级令牌名）；对比度按 CODE-POLICY 取证规范重跑一次浅/暗真机实测 | 官方 `packages/client/ui-theme/src/styles/` 里 `--dsw-alias-label-*` 与 `--dsw-alias-link` 的取值 | 已实测（task-67 真机浅/暗采样）；承重文本已抬到 secondary，这两处按官方层级保留并**建议向官方反馈**浅色对比度 |
...
| E4 | 启动就绪判据不是退出码：`dsh --profile <p>` 以任务模式跑时只输出 `dsh: a task is required`（树挂载成功） | 真机脚本的就绪探测（`tools/e2e-visual.mjs`、`tools/e2e-lifecycle.mjs` 用端口/日志而非退出码） | **静默**：判据失效 → 脚本假绿（把没起来的环境当成起来了） | `node tools/e2e-lifecycle.mjs` | docs/private/test-env-feasibility.md:146 | 已实测（他人台账 + task-62 复跑） |
## 2. 最小 smoke 序列（官方升级后按顺序跑）

前提：官方 dsh 的 `lib/bin.js` 能解析到（`e2e-lifecycle.mjs` 自带解析链，可用 `DSH_E2E_BIN` 显式指定）；视觉 e2e（`tools/e2e-visual.sh`）仍需 `~/.dsh/profiles/pm-test` 作为源 profile。端口占用见 CODE-POLICY §7.7 的号段约定。
...
| 4 | `node tools/e2e-lifecycle.mjs`（`bash tools/e2e-lifecycle.sh` 是兼容壳） | 退出码 0、14 项全 PASS（"建环境 → 启动 → 可达 → 停止"全链路；自建 profile、零 bash） | B6/B7/C3/E4（真建环境、真启动、就绪判据） |
...
| U3 | B6「建环境链路」 | **已补证（task-62）**：Node 版 `tools/e2e-lifecycle.mjs` 自建 profile 真机 14/14、退出码 0（含无 PATH 复跑） | —（保留此行以记录它曾经是缺口） |
